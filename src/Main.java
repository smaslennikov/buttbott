...
public class Main {
	...
	static boolean startMe() {
		try {
			...
			sendToIRC("NICK " + NICK + "\r\n");
			sendToIRC("USER " + USER + "\r\n");
			sendToIRC("PRIVMSG nickserv :identify NOPE\r\n");
	...
	static boolean replyHamburglar(String tempLine) {
		System.err.print(" >>> Hamburglar message ");
		try {
			...
			if (tnick.contains(NICK))
				sendToIRC("PRIVMSG " + sourceChan + " :" + "I am a hamburglar.\r\n");
			...
			System.err.println("Bad substring in replyHamburglar");
		}
		return false;
	}
}
